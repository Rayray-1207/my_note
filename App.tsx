...
export default function App() {
  ...
  const renderVoiceInput = () => (
    <div className="fixed inset-0 bg-neu-base z-50 flex flex-col items-center justify-center">
        <div className="absolute top-12 left-6">
           <NeuButton onClick={() => { stopListening(); setView('HOME'); }} icon={X} />
        </div>
        
        <div className="absolute top-12 right-6 font-mono text-lg font-bold text-green-600 px-4 py-2 rounded-xl shadow-neu-pressed-sm bg-neu-base">
            00:{timeLeft < 10 ? `0${timeLeft}` : timeLeft}
        </div>

        <div className="mb-16 flex flex-col items-center gap-4">
             <p className="text-2xl font-bold text-neu-text">
                 {isListening ? "Xyla 正在聆听..." : "点击开始"}
             </p>
             <p className="text-sm text-slate-400">
                {isListening ? "再次点击停止" : "限时 60 秒"}
             </p>
        </div>

        <div className="flex items-center gap-8 relative">
             {isListening ? (
                 <button onClick={handleVoiceStop} className="w-32 h-32 rounded-full bg-neu-base shadow-neu-flat flex items-center justify-center text-green-500 animate-pulse relative z-10 active:shadow-neu-pressed transition-all">
                    <div className="w-10 h-10 bg-green-500 rounded-lg shadow-neu-flat"></div>
                 </button>
             ) : (
                 <button onClick={startListening} className="w-32 h-32 rounded-full bg-neu-base shadow-neu-flat flex items-center justify-center text-green-500 active:shadow-neu-pressed transition-all duration-200 hover:text-green-600">
  ...
  const renderWaterfallView = () => (
    <div className="h-full overflow-y-auto p-4 pb-32 no-scrollbar animate-fade-in">
      <div className="columns-2 gap-4 space-y-4">
         {filteredRecords.slice().reverse().map(r => (
            <div key={r.id} onClick={() => { setEditingRecord(r); setView('EDIT_RECORD'); }} className="break-inside-avoid bg-neu-base rounded-2xl shadow-neu-flat overflow-hidden active:shadow-neu-pressed transition-all duration-200 p-2">
               {(r.mediaMeta?.coverUrl || r.originalImage) && (
                  <div className="w-full aspect-auto bg-neu-base rounded-xl overflow-hidden shadow-neu-pressed-sm mb-2 relative">
                     <img 
                        src={r.mediaMeta?.coverUrl || r.originalImage} 
               ...
               <div className="p-2">
                  <div className="flex justify-between items-start mb-1">
                      <h3 className="font-bold text-neu-text text-sm line-clamp-2 flex-1 mr-1">
                         {r.type === RecordType.NOTE ? r.topic : r.mediaMeta?.title}
                      </h3>
                  </div>
                  <p className="text-[10px] text-slate-400 mb-2 font-mono">{formatDateTime(r.timestamp)}</p>
                  <p className="text-xs text-slate-500 line-clamp-3 mb-2 leading-relaxed font-light">
                     {r.content}
                  </p>
                  <div className="flex flex-wrap gap-1">
                        {r.keywords.slice(0, 2).map((k, i) => (
                           <button 
                             key={i}
                             onClick={(e) => handleKeywordSearch(e, k)}
                             className="px-2 py-1 bg-neu-base shadow-neu-flat text-slate-500 hover:text-green-600 rounded-lg text-[10px] whitespace-nowrap max-w-[80px] truncate active:shadow-neu-pressed"
                           >
                             #{k}
                           </button>
                        ))}
  ...
  const renderListView = () => (
    <div className="h-full overflow-y-auto p-4 pb-32 no-scrollbar animate-fade-in space-y-4">
       {filteredRecords.slice().reverse().map(r => (
          <div key={r.id} onClick={() => { setEditingRecord(r); setView('EDIT_RECORD'); }} className="bg-neu-base rounded-2xl shadow-neu-flat p-4 active:shadow-neu-pressed transition-all flex gap-4">
             {(r.mediaMeta?.coverUrl || r.originalImage) && (
                <div className="w-16 h-16 rounded-xl shrink-0 bg-neu-base shadow-neu-pressed-sm overflow-hidden">
                   <img src={r.mediaMeta?.coverUrl || r.originalImage} className="w-full h-full object-cover" alt="thumb" />
                </div>
             )}
             <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start mb-1">
                    <h3 className="font-bold text-neu-text text-sm truncate pr-2">{r.type === RecordType.NOTE ? r.topic : r.mediaMeta?.title}</h3>
                    <span className="text-[10px] text-slate-400 shrink-0 font-mono">{formatDateTime(r.timestamp)}</span>
                </div>
                <p className="text-xs text-slate-500 line-clamp-2 mb-2">{r.content}</p>
  ...
  return (
    <div className="max-w-md mx-auto h-screen bg-neu-base shadow-2xl flex flex-col relative overflow-hidden font-sans text-neu-text">
       {/* Overlays */}
       {isProcessing && (
          <div className="fixed inset-0 z-[60] bg-neu-base/80 backdrop-blur-sm flex flex-col items-center justify-center">
             <Loader2 className="w-10 h-10 text-green-500 animate-spin mb-4" strokeWidth={2} />
             <p className="text-slate-500 font-medium animate-pulse">Xyla 正在思考...</p>
          </div>
       )}
       ...
       <div className="flex-1 relative">
          ...
          {view === 'LIST' && (
             homeMode === 'CALENDAR' ? (
                <div className="h-full overflow-y-auto pb-32 no-scrollbar animate-fade-in">
                   <CalendarView records={filteredRecords} onSelectRecord={(r) => { setEditingRecord(r); setView('EDIT_RECORD'); }} />
                </div>
             ) : homeMode === 'WATERFALL' ? renderWaterfallView() : renderListView()
          )}

          {view === 'SEARCH' && (
             <div className="absolute inset-0 bg-neu-base z-20 flex flex-col animate-fade-in">
                <div className="p-6 flex items-center gap-4">
                   <NeuButton onClick={() => setView('HOME')} icon={ArrowLeft} />
                   <div className="flex-1 h-12 bg-neu-base rounded-full shadow-neu-pressed flex items-center px-4">
                       <Search size={18} className="text-slate-400 mr-2"/>
                       <input 
                          autoFocus
                          className="flex-1 bg-transparent focus:outline-none text-neu-text placeholder-slate-400"
                          placeholder="搜索记忆..."
                          value={searchQuery}
                ...
                <div className="flex-1 overflow-y-auto px-6 pb-6 no-scrollbar">
                   {filteredRecords.map(r => (
                      <div key={r.id} onClick={() => { setEditingRecord(r); setView('EDIT_RECORD'); }} className="p-4 mb-4 bg-neu-base shadow-neu-flat rounded-2xl active:shadow-neu-pressed transition-all cursor-pointer">
                         <div className="flex justify-between mb-2">
                            <span className="font-bold text-neu-text text-sm">{r.type === 'NOTE' ? r.topic : r.mediaMeta?.title}</span>
                            <span className="text-[10px] text-slate-400">{formatDateTime(r.timestamp)}</span>
                         </div>
                         <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed">{r.content}</p>
                      </div>
                   ))}
                </div>
             </div>
       ...
       {/* Persistent Bottom Bar - Visible on HOME and LIST views */}
       {(view === 'HOME' || view === 'LIST') && renderBottomBar()}

       {/* Modals */}
       {view === 'EDIT_RECORD' && renderEditRecord()}
       {view === 'INPUT_VOICE' && renderVoiceInput()}
    </div>
  );
}
